...
description: 從任何已安裝的 Electron 應用程式（.asar 套件）中提取資源與 JavaScript，當有 .js.map 檔案時還原原始碼，否則使用 Prettier 格式化壓縮後的程式碼。當使用者想要提取 Electron 應用程式、反編譯 Electron、取得原始碼、檢查 .asar、看 Electron 應用原始碼，或詢問桌面 Electron 應用程式如何建置時使用。跳過 node_modules，支援 macOS 與 Windows。
version: 1.119.0
metadata:
  openclaw:
