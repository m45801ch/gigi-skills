...
description: 透過 URL 或影片 ID 下載 YouTube 影片逐字稿/字幕與封面圖片。支援多種語言、翻譯、章節與發言者識別。快取原始資料以便快速重新格式化。當使用者要求取得 YouTube 逐字稿、下載字幕、取得字幕、YouTube 字幕、YouTube 封面、影片封面，或提供 YouTube URL 並希望提取逐字稿/字幕文字或封面圖片時使用。
version: 1.1.0
metadata:
  openclaw:
