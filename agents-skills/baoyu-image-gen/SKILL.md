...
description: 使用 OpenAI GPT Image 2、Azure OpenAI、Google、OpenRouter、DashScope、Z.AI GLM-Image、MiniMax、Jimeng、Seedream、Replicate 與 Agnes API 進行 AI 圖片生成。支援文字轉圖片、參考圖片、長寬比，以及從已儲存的提示詞檔案進行批次生成。預設為循序生成；當使用者已有多個提示詞或需要穩定的多圖片產出時，使用批次並行生成。當使用者要求生成、建立或繪製圖片時使用。
version: 2.1.0
metadata:
  openclaw:
