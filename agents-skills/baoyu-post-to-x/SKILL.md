...
description: 將內容和文章發佈到 X（Twitter）。支援包含圖片/影片的一般貼文以及 X Articles（長篇 Markdown）。在 Codex 中，對於明確要求 Codex Chrome 插件的情況，使用 Chrome Extension 工作流程；否則使用 Chrome Computer Use，僅在被允許時回退到真實的 Chrome CDP 腳本。當使用者要求發文到 X、推文、發布到 Twitter 或分享到 X 時使用。
version: 1.58.1
metadata:
  openclaw:
