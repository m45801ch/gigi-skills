...
description: 將 Markdown 轉換為帶有微信相容主題的樣式化 HTML。支援程式碼高亮、數學公式、Mermaid（透過無頭 Chrome 渲染為 PNG）、PlantUML、註腳、提示框、資訊圖表，以及可選的外部連結底部引用。當使用者要求 markdown 轉 html、md 轉 html、微信外鏈轉底部引用，或需要從 markdown 產生樣式化 HTML 輸出時使用。
version: 1.117.3
metadata:
  openclaw:
