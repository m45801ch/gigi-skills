...
description: 使用本地 wx-cli 二進位檔將微信群組聊天精華彙整為結構化摘要。預設生成一般摘要；毒舌版為選擇加入。跨執行階段維護每個群組的歷史記錄、使用者個人檔案，以及群組事實記憶，內建隱私保護。當使用者要求總結群聊、群聊精華、群聊摘要、summarize group chat、group chat digest，提到微信群組名稱與時間範圍，或要求回溯畫像、初始化畫像時使用。當使用者說毒舌版、roast 版或再來個毒舌的時，加入毒舌版本。
version: 1.117.4
metadata:
  openclaw:
