...
description: 透過 API 或 Chrome CDP 將內容發布到微信公眾號。支援使用 HTML、markdown 或純文字輸入的文章發布，以及包含多張圖片的貼圖發布（原圖文）。Markdown 文章流程預設將一般外部連結轉換為底部引用，以產生微信友善的輸出。當使用者提到發布公眾號、post to wechat、微信公眾號或貼圖/圖文/文章時使用。
version: 1.118.2
metadata:
  openclaw:
