...
description: 使用 baoyu-fetch CLI（Chrome CDP 搭配站點專用適配器）取得任何 URL 並轉換為 markdown。內建 X/Twitter、YouTube 逐字稿、Hacker News 討論串以及透過 Defuddle 處理一般頁面的適配器。透過互動等待模式處理登入/CAPTCHA。當使用者想要將網頁儲存為 markdown 時使用。
version: 1.61.0
metadata:
  openclaw:
